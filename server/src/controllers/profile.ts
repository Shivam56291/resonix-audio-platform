...
export const getPlaylistAudios: RequestHandler = async (req, res) => {
  ...
  const skip = parseInt(pageNo) * parseInt(limit);
  const perPage = parseInt(limit);

  const playlistAggregation = [
    {
      $match: {
        _id: new Types.ObjectId(playlistId),
        visibility: { $ne: "private" },
    ...
    {
      $project: {
        items: { $slice: ["$items", skip, perPage] },
        title: 1,
        owner: 1,
      },
    },
  ...
  const [playlistResult] = await Playlist.aggregate(playlistAggregation);

  if (playlistResult) {
    return res.status(200).json({ list: playlistResult });
  }

  const autoPlaylistAggregation = [
    {
      $match: {
        _id: new Types.ObjectId(playlistId),
      },
